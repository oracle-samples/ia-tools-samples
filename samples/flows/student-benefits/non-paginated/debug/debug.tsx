...
async function getBearerToken(hubURL: string, apiVersion: string, client: string, secret: string) {
    ...
    try {
        ...
        throw new Error("Response parsing not allowed, check console for details (probably a CORS issue)");
    }
...
function Input(props: InputProperties) {
    return props.hidden ? null : <><label for={props.id}>{props.text}:</label>&nbsp;<input id={props.id} type={props.type ?? "text"} style={props.style ?? "text"} value={props.value} required={props.required} disabled={false} onChange={e => props.valueUpdate((e.target as HTMLInputElement).value)} size={props.size}></input><br/></>
}
...
function FlowDetailsForms() {
        ...
        if (hubURL && apiVersion && clientId && clientSecret) {
            ...
            getBearerToken(hubURL, apiVersion, clientId, clientSecret)
                    .then(token => {
                        ...
                        if (wdURL === null) {
                            setWdURL(hubURL.substring(0, hubURL.indexOf("/", 9)) + "/web-determinations")
                        }
                    })
    ...
    return <div>
        <form onSubmit={e => e.preventDefault()} style={"padding-bottom:20px"} disabled={bearerToken}>
            <strong>Get a bearer token</strong><br/>
            <Input text="Hub" id="hub" value={hubURL} valueUpdate={setHubURL} size={40}></Input>
        ...
        {bearerToken !== null ? <>Bearer token:<pre>{bearerToken}</pre></> : null}

        <form hidden={!bearerToken} onSubmit={e => e.preventDefault()} style={"padding:20px 0"} disabled={bearerToken}>
            <strong>Use the bearer token to start a flow action</strong><br/>
            <Input text="Web-determinations URL" id="wd" value={wdURL} valueUpdate={setWdURL} size={40}></Input>
        ...
        <div hidden={!jwt}>
            ...
            <pre hidden={!globalDataSetJson}>{globalDataSetJson}</pre>
        </div>
